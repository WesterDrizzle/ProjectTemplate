...
{	
	...
	GPIO_Init(GPIOG, &GPIO_InitStructure);
 	...
 	FSMC_ReadWriteNORSRAMTiming.FSMC_AddressSetupTime = 0x00;	 //地址建立时间（ADDSET）为1个HCLK 0.013us = 13ns
	...
	FSMC_ReadWriteNORSRAMTiming.FSMC_AccessMode = FSMC_AccessMode_A;	 //模式A//访问速度更快 
    
//使用FSMC外接存储器时，其存储单元是映射到STM32的内部寻址空间的
