...
u8 TP_Read_XY(u16 *x, u16 *y)
{
    ...
    {
        xtemp = TP_Read_XOY(0X90);   /* 读取X轴坐标AD值, 并进行方向变换 */
        ytemp = TP_Read_XOY(0XD0);   /* 读取Y轴坐标AD值 */
    }
    else                            /* X,Y方向与屏幕相同 */
    {
        xtemp = TP_Read_XOY(0XD0);   /* 读取X轴坐标AD值 */
        ytemp = TP_Read_XOY(0X90);   /* 读取Y轴坐标AD值 */
    }
	*x = xtemp;
	*y = ytemp;
	return 1; // 读数成功
...
u8 TP_Init(void)
{
	...
	defined(TFTLCD_R61509VE) || defined(TFTLCD_SSD1963N) || defined(TFTLCD_ILI9488)
	tp_dev.touchtype = 0;
	tp_dev.touchtype |=tftlcd_data.dir & 0X01;
